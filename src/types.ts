...
export type Transformer<T extends HTMLElement, V> = {
  ...
  // parse() turns content attribute values into IDL attribute values. Must
  // never throw exceptions, and instead always just deal with its input. Must
  // not cause any observable side effects. May return NO_VALUE in case the
  // content attribute can't be parsed, in which case the @attr() decorator must
  // not change the IDL attribute value
  parse: (this: T, rawValue: string | null) => V | typeof NO_VALUE;
  // Validates and possibly transforms setter inputs, which may be of absolutely
  // any type. May throw for invalid values, just like setters on built-in
  // elements may. Must not cause any observable side effects.
  ...
  // always returns true.
  updateContentAttr: (oldValue: V | null, newValue: V | null) => boolean | null;
};
